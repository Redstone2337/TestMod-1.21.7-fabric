...
public record FreezingSwordComponent(int chargeProgress, boolean isCharging, int charges) implements TooltipAppender {
    ...
    public void appendTooltip(Item.TooltipContext context, Consumer<Text> textConsumer, TooltipType type, ComponentsAccess components) {
        ...
        textConsumer.accept(Text.translatable("tooltip.freezing_sword.desc").formatted(Formatting.GRAY));
        textConsumer.accept(Text.empty());

        if (isCharging) {
            textConsumer.accept(Text.translatable("tooltip.freezing_sword.charging",
                    (int)(chargePercent * 100)).formatted(Formatting.BLUE));
        } else {
        ...
        textConsumer.accept(Text.translatable("tooltip.freezing_sword.damage.normal",
                FreezeSwordItem.BASE_DAMAGE).formatted(Formatting.GREEN));

        textConsumer.accept(Text.translatable("tooltip.freezing_sword.damage.charged",
                FreezeSwordItem.calculateDamage(charges, true)).formatted(Formatting.RED));

        textConsumer.accept(Text.translatable("tooltip.freezing_sword.damage.non_boss",
                FreezeSwordItem.calculateDamage(charges, false)).formatted(Formatting.LIGHT_PURPLE));

        if (charges >= FreezeSwordItem.MAX_CHARGES) {
            textConsumer.accept(Text.translatable("tooltip.freezing_sword.max_charges")
